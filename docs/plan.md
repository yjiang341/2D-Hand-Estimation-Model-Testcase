...
Steps
Step 0: Freeze data contract for a single pose frame.
...
Step 1.1: First milestone you requested (x,y only + normalization).
...
Step 1.2: Packet serialization format.
...
Step 1.3: Unit tests for preprocessing and packet I/O.
Validate:
...
Step 2.4: Skeleton renderer to MP4 (later the module also need to render in realtime during Zoom or google meets).
Render recovered keypoints on black canvas and export MP4 at target FPS for intelligibility checks.

Step 3: Live webcam path.
